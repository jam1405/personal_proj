#This file deals with solving the numerous differential equations required for the simulation
import numpy as np 
import os

#define various classes

#Boundary dictates the type and value of a boundary (such as a pipe)
class Boundary:
...
#Space is related to a boundary as the inside of a pipe is related to the pipe itself
class Space:
...
#Fluid class which represents fluid properties (viscosity mu, density rho)
class Fluid:
    def __init__(self,rho,mu) -> None:
...
#All functions below takes previous class objects as arguements
#First to define boundary conditions for horizontal velocity

def SetUBoundary(space:Space,left:Boundary,right:Boundary,top:Boundary,bottom:Boundary):
    if(left.type == "D"): #DIRICHLET BOUNDARY COND.
...
def SetVBoundary(space:Space, left:Boundary, right:Boundary, top:Boundary, bottom:Boundary):
    if(left.type=="D"):
